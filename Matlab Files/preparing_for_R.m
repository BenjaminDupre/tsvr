...
for prsnj = 1:size(all2,2) % this instruct to go over all participants
    ...
    for line=1:size(data,1)-1 % this indicates to go over every row
        A(line,3) = data.correctCounter(line+1)~=data.correctCounter(line); % this looks for diference in correct counter
...
mistakes_ag = array2table(mistakesf, 'VariableNames',{'set','level','mistake','stimuli','participant'}); % this adds names 
...
%% ============== Saving Mistakes =========================================
writetable(behavioral_a,[ allFolder 'responsetime.csv'])
toc